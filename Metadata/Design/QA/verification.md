...
- Follow-up after a device screenshot showed a saved thumbnail together with a `1` retry badge: the badge is the count of local encoded videos still believed to need a Photos retry. A stale in-memory URL could outlive its already-removed local file; tapping it then produced the misleading “video file could not be found” alert even though the successful video in Photos was unaffected.
- Pending entries now reconcile against readable files before the badge is shown or retry begins, de-duplicate by standardized filesystem path, and remove successful entries by that same canonical path. A missing retry copy is silently discarded because there is no recoverable local file and it does not prove the Photos save failed. The obsolete missing-file alert text was removed.
- Added and passed a simulator regression that loads one pending entry, removes its local file to reproduce the stale state, invokes retry, and verifies the count becomes zero without presenting an error. All existing denial, retention, retry and subsequent-recording checks continue to pass.
